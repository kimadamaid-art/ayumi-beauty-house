...
export async function getCachedPosCatalog(forceRefresh = false) {
    ...
    pending = (async () => {
        try {
            ...
            const [trRes, prodRes, cpRes, thRes, catRes, wkRes] = await Promise.all([
                ...
                // Daftar worker aktif: dipakai kasir untuk memilih siapa yang mengerjakan
                // tindakan yang punya upah worker (nominal rupiah, bukan persen).
            ...
            const allOk = !trRes.error && !prodRes.error && !cpRes.error && !thRes.error && !catRes.error && !wkRes.error
            if (allOk) {
