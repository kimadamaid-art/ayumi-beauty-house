...
export function getFriendlyErrorMessage(error) {
    ...
    // 2. Error Supabase Auth: Email sudah terdaftar / duplikat
    if (
        lowerMessage.includes('already been registered') ||
        lowerMessage.includes('already registered') ||
        lowerMessage.includes('email already in use') ||
        lowerMessage.includes('email address has already been registered')
    ) {
        return 'Email ini sudah terdaftar di sistem. Silakan gunakan email lain atau edit user yang bersangkutan.';
    }
    ...
    // 3. Kode 23505 / "duplicate key" -> Data ganda
    ...
    if (message) {
        return `Terjadi kesalahan sistem: ${message}. Silakan coba lagi atau hubungi admin.`;
    }
