...
export function getCommissionBasePrice(item, proportionalCouponPrice = null) {
    ...
    // 3b. Cek apakah ada catatan KUPON_LAMA di notes: [KUPON_LAMA:itemId:name]
    const matchOldPkg = rawNotes.match(/\[KUPON_LAMA:([^:]+):([^\]]+)\]/)
    if (matchOldPkg && proportionalCouponPrice && typeof proportionalCouponPrice === 'object') {
        const couponItemId = matchOldPkg[1]
        if (proportionalCouponPrice[couponItemId]) {
            return Number(proportionalCouponPrice[couponItemId])
        }
    }
...
/**
 * Cek apakah tindakan merupakan tindakan infus / worker (nakes)
 * Semua tindakan infus hanya dapat dilakukan oleh Worker dengan komisi terapis 0%
 */
export function isInfusionTreatment(treatmentName = '', notes = '') {
    const combined = `${treatmentName || ''} ${notes || ''}`.toLowerCase()
    // Kecualikan facial Oxy Infusion yang merupakan treatment wajah terapis
    if (combined.includes('oxy infus')) return false
    return /infused/i.test(combined) || /\binfus\b/i.test(combined) || combined.includes('[worker]')
}
...
export function calculateTherapistCommission(item, proportionalCouponPrice = null) {
    ...
    // Guard: Tindakan worker / infus tidak pernah memberikan komisi ke terapis
    if (item.performer_type === 'worker' || item.is_worker) return 0
    if (item.notes && item.notes.includes('[WORKER]')) return 0
    const tName = item.treatments?.name || item.name || item.treatment_name || ''
    if (isInfusionTreatment(tName, item.notes)) return 0
        ...
        if (cl.treatment_record_id && cl.patient_coupon_items) {
            ...
            if (tx) {
                ...
                if (itemSubtotal > 0 && txSubtotal > 0 && txDiscount > 0) {
                    // Ada diskon transaksi di kasir: hitung proporsional harga riil bersih kupon (tanpa fee QRIS/payment)
                    const netSubtotal = Math.max(0, txSubtotal - txDiscount)
                    const discountRatio = netSubtotal / txSubtotal
                    purchasePrice = Math.round(itemSubtotal * discountRatio)
                } else if (itemSubtotal > 0) {
                    // Gunakan harga bersih item kupon (sebelum surcharge pembayaran/QRIS)
                    purchasePrice = itemSubtotal
                } else if (txSubtotal > 0) {
                    // Jika tidak ada item kupon spesifik, gunakan subtotal transaksi dikurang diskon
                    purchasePrice = Math.max(0, txSubtotal - txDiscount)
                }
            }
            ...
            if (purchasePrice > 0 && totalSessions > 0) {
                const perSession = Math.round(purchasePrice / totalSessions)
                if (cl.treatment_record_id) {
                    couponMap[cl.treatment_record_id] = perSession
                }
                if (cl.patient_coupon_item_id) {
                    couponMap[cl.patient_coupon_item_id] = perSession
                }
            }
        }
    })
