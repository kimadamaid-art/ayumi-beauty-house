...
export function getItemCategory(item, itemType = 'product') {
    ...
    if (itemType === 'coupon') {
        if (!item.category || item.category === 'VIP') {
            return 'PAKET KUPON VIP'
        }
        return item.category.toUpperCase().startsWith('PAKET') ? item.category.toUpperCase() : `PAKET KUPON ${item.category.toUpperCase()}`
    }
    ...
    if (Array.isArray(product.variants) && product.variants.length > 0) {
        return product.variants
            ...
            .map(v => ({
                ...
                stocks: v.stocks && typeof v.stocks === 'object' ? v.stocks : {}
            }))
    }
    ...
    if (Array.isArray(variants) && variants.length > 0) {
        const validVariants = variants
            ...
            .map(v => {
                ...
                if (v.stocks && typeof v.stocks === 'object' && Object.keys(v.stocks).length > 0) {
                    varObj.stocks = v.stocks
                }
                return varObj
            })
...
/**
 * Ambil stok varian untuk cabang tertentu
 */
export function getVariantStock(variant, branchId) {
    if (!variant || !branchId || !variant.stocks) return 0
    return Math.max(0, parseInt(variant.stocks[branchId], 10) || 0)
}

/**
 * Hitung total stok cabang dari seluruh varian
 */
export function calculateBranchTotalFromVariants(variants, branchId) {
    if (!Array.isArray(variants) || !branchId) return 0
    return variants.reduce((sum, v) => sum + getVariantStock(v, branchId), 0)
}

/**
 * Ambil harga normal / sebelum diskon dari suatu item produk & variannya
 */
export function getProductOriginalPrice(item, product) {
    const prod = product || item?.products
    if (!prod) return Number(item?.original_price) || Number(item?.price) || 0
    const basePrice = Number(prod.price) || 0
    const variants = getProductVariants(prod)
    if (variants.length > 0 && item?.name) {
        const itemLower = item.name.toLowerCase()
        const matchedVariant = variants.find(v => v.name && itemLower.includes(v.name.toLowerCase()))
        if (matchedVariant && Number(matchedVariant.price) > 0) {
            return Number(matchedVariant.price)
        }
    }
    return basePrice > 0 ? basePrice : (Number(item?.original_price) || Number(item?.price) || 0)
}
