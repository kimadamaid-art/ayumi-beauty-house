...
/**
 * Kirim notifikasi ke Admin & Kasir saat Terapis Siap Menerima Pasien
 * (Owner TIDAK disertakan agar tidak terganggu oleh notifikasi operasional klinik)
 */
export async function notifyTherapistReady({ supabase, appointment, therapistUser }) {
    ...
    try {
        ...
        // Ambil semua admin & kasir di cabang terkait (role owner dikecualikan)
        const { data: staffUsers, error: uErr } = await supabase
            .from('users')
            .select('id, role, branch_id')
            .in('role', ['admin', 'kasir', 'receptionist'])
            .eq('is_active', true)

        if (uErr || !staffUsers) {
            console.error('Error fetching admin/kasir users:', uErr)
            return false
        }

        // Filter: Hanya staf cabang yang sama atau yang tidak terikat cabang (global)
...
/**
 * Kirim notifikasi ke Admin & Kasir saat Treatment Selesai (Input Rekam Medis Selesai)
 * (Owner TIDAK disertakan agar tidak terganggu oleh notifikasi operasional kasir)
 */
export async function notifyTreatmentCompleted({ supabase, appointment, performerUser }) {
    ...
    try {
        ...
        // Ambil staf admin/kasir/receptionist di cabang terkait (role owner dikecualikan)
        const { data: staffUsers, error: uErr } = await supabase
            .from('users')
            .select('id, role, branch_id')
            .in('role', ['admin', 'kasir', 'receptionist'])
        ...
        if (uErr || !staffUsers) {
            console.error('Error fetching admin/kasir users for treatment completion:', uErr)
            return false
        }
...
/**
 * Kirim notifikasi ke Admin & Owner saat stok produk/varian tersisa <= 5 unit
 */
export async function notifyLowStock({
    ...
    if (!productId || remainingStock === undefined || remainingStock === null) return false

    const stockQty = Number(remainingStock)
    if (isNaN(stockQty) || stockQty > 5) return false
    ...
    try {
        ...
        console.error('Failed to call /api/notifications/low-stock:', err)
        return false
    }
}

/**
 * Pindai seluruh stok produk di database dan kirim notifikasi jika ada yang <= 5 unit
 */
export async function checkAllLowStocks() {
    try {
        const response = await fetch('/api/notifications/low-stock', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        console.error('Failed to scan low stocks:', err)
        return false
    }
}
