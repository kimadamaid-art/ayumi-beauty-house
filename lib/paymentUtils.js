...
export function getQrisFee(tx) {
    ...
    // Pembayaran tunggal QRIS
    if (method === 'qris') {
        if (netBase > 0 && total > netBase) {
            return total - netBase
        }
        if (netBase > 0) {
            return Math.round(netBase * 0.003)
        }
        return Math.round(total - (total / 1.003))
    }

    // Pembayaran Split dengan QRIS
    if (method === 'split' || notes.includes('[SPLIT:')) {
        ...
        if (rawSplits.qris > 0) {
            ...
            const mdrFromRaw = Math.round(rawSplits.qris - (rawSplits.qris / 1.003))
            return mdrFromRaw > 0 ? mdrFromRaw : Math.round(rawSplits.qris * 0.003)
        }
    }
...
export function getNetTransactionRevenue(tx) {
    ...
    if (method === 'split' || notes.includes('[SPLIT:')) {
        ...
        if (rawSplits.qris > 0) {
            ...
            if (total > netBase && fee > 0) {
                return Math.max(0, total - fee)
            }
            return netBase > 0 ? netBase : Math.max(0, total - fee)
        }
    }
