...
const sqlFile = path.join(__dirname, 'migration_all_rls.sql');
const sqlContent = fs.readFileSync(sqlFile, 'utf8');

async function run() {
    console.log('--- EXECUTING SUPABASE RLS MIGRATION ---');
    ...
    if (supabaseUrl && serviceRoleKey) {
        ...
        try {
            ...
            if (error) {
                ...
                console.log('\nMake sure the "exec_sql" RPC function is created in your database:');
                console.log(`
...
$$;
    ...
    console.log('Sebab:');
    console.log('1. SUPABASE_SERVICE_ROLE_KEY tidak ditemukan di .env.local (hanya ada anon key yang tidak memiliki hak akses DDL)');
    console.log('2. DATABASE_URL (PostgreSQL Connection String) tidak didefinisikan.');
    console.log('\nTindakan yang dapat Anda lakukan:');
    console.log('A. Silakan salin konten dari file scratch/migration_all_rls.sql dan tempel di Supabase SQL Editor (DASHBOARD) Anda.');
    console.log('B. ATAU tambahkan DATABASE_URL atau SUPABASE_SERVICE_ROLE_KEY ke dalam .env.local, lalu jalankan kembali perintah ini:');
    console.log('   node scratch/execute_rls_migration.js');
}
