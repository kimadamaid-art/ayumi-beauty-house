...
export async function DELETE(request, { params }) {
    try {
        ...
        // 8.5. If this transaction has a linked treatment_record_id, check if it was directly generated by cashier or has no other transactions
            ...
            try {
                ...
                if (!otherTxs || otherTxs.length === 0) {
                    const { data: tr } = await supabaseAdmin
                        ...
                        .select('id, result_notes')
                        .eq('id', tx.treatment_record_id)
                        .maybeSingle()

                    if (tr && (tr.result_notes?.includes('Tindakan Kasir Langsung') || tr.result_notes?.includes('Worker') || tr.result_notes?.includes('Kasir'))) {
                        await supabaseAdmin.from('treatment_record_items').delete().eq('treatment_record_id', tr.id)
                        await supabaseAdmin.from('treatment_records').delete().eq('id', tr.id)
                    }
                }
            } catch (trCleanErr) {
                console.warn('Warning cleaning up linked cashier treatment record:', trCleanErr.message)
            }
        }
